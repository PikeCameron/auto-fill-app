...
module.exports.verify = function verify(email, options, callback) {
  ...
  if (
    ...
    typeof params.options.email === "string"
  )
      ...
      switch (stage) {
        ...
        case 3:
          if (
            ...
            (params.options.ignore &&
              response.indexOf(params.options.ignore) > -1)
          ) {
            // RCPT Worked
            success = true;

            // Check if the response indicates a catch-all domain
            if (
              response.indexOf("250") > -1 &&
              response.indexOf("No such user") > -1
            ) {
              catchAll = true;
            }
          }
